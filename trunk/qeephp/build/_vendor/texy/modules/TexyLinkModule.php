 ...
 * @link       http://texy.info
 * @package    Texy
 * @version    $Id: TexyLinkModule.php 11 2012-02-08 06:05:02Z zys4416@gmail.com $
 */
...
final class TexyLinkModule extends TexyModule
{
	...
	private function textualUrl($link)
	{
		...
		if ($this->shorten && preg_match('#^(https?://|ftp://|www\.|/)#i', $link->raw)) {
			...
			if (isset($parts['path']))
				$res .=  (iconv_strlen($parts['path'], 'UTF-8') > 200 ? ("/\xe2\x80\xa6" . iconv_substr($parts['path'], -12, 12, 'UTF-8')) : $parts['path']);

			if (isset($parts['query'])) {
				$res .= iconv_strlen($parts['query'], 'UTF-8') > 200 ? "?\xe2\x80\xa6" : ('?' . $parts['query']);
			} elseif (isset($parts['fragment'])) {
				$res .= iconv_strlen($parts['fragment'], 'UTF-8') > 200 ? "#\xe2\x80\xa6" : ('#' . $parts['fragment']);
			}
			return $res;
		}
