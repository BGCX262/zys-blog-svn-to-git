<?php
// $Id: ticket.php 11 2012-02-08 06:05:02Z zys4416@gmail.com $

/**
 * Ticket 封装来自 tickets 数据表的记录及领域逻辑
 */
class Ticket extends QDB_ActiveRecord_Abstract
{
    const STATUS_SOLD = 1;
    const STATUS_IDLE = 0;
    ...
    static function __define()
    {
        ...
        (
            ...
            (
                'meeting_id' => array
                (
                    array('is_int', '会议ID必须是一个整数'),

                ),

                'type_name' => array
                (
                    array('not_empty', '入场卷类型不能为空'),
                    array('max_length', 30, '入场卷类型不能超过 30 个字符'),

                ),

                'number' => array
                (
                    array('not_empty', '入场卷编号不能为空'),
                    array('max_length', 20, '入场卷编号不能超过 20 个字符'),
